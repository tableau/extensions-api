...
import { getFieldByFieldName } from '../metadata/dependencyGraph';
...
describe('buildFieldIndex', () => {
  ...
  it('includes parameters, sets, and calculated fields (excludes action auto-columns)', () => {
    ...
    expect(fieldTypes.has('set')).toBe(true);
  ...
  it('classifies Segment Set as set', () => {
    ...
    expect(segmentSet?.fieldType).toBe('set');
    expect(segmentSet?.fieldName).toBe('Segment Set');
  });
  ...
  it('resolves Sales Forecast upstream dependencies', () => {
    const salesForecast = getFieldByFieldName(index.fields, 'Sales Forecast');
  ...
  it('marks Segment used via set, sheets, and parameter source-field', () => {
  ...
  it('stores column default-format and visual-totals on Profit per Order', () => {
    const profitPerOrder = getFieldByFieldName(index.fields, 'Profit per Order');
