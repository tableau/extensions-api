...
function fieldDisplayLabel(fieldName: string | null, fieldId: string): string {
  if (fieldName) {
    return fieldName;
  }
...
function resolveParameterSourceFields(
  ...
  for (const child of parametersDatasource.children ?? []) {
    ...
    const sourceFieldId = resolveReferenceString(sourceFieldRef, fields, null);
...
export function buildFieldCatalog(workbook: WorkbookNode): Map<string, FieldRecord> {
  ...
  if (parametersDatasource) {
    resolveParameterSourceFields(fields, parametersDatasource);
  }
...
export function findFieldByFieldId(
  ...
  return Array.from(fields.values()).find((field) => field.fieldId === fieldId);
}
